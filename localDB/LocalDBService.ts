...
class LocalDBService {
    ...
    try {
      this.db.transaction((tx: any) => {
        contacts.forEach((contact) => {
          tx.execute(`
            INSERT INTO contacts (phoneNumber, firebase_uid, name, photo, isRegistered, chatInit, lastSync, unreadCount, lastMessage, chatTimestamp)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  public deleteMessagesForChat(chatId: string) {
    try {
      this.db.execute('DELETE FROM messages WHERE chatId = ?', [chatId]);
    } catch (error) {
